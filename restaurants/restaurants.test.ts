...
test('post /restaurants', async () => {
    try{
        let response = await request(address).post('/restaurants').send({
            name: 'RestaurantTest'
        });
        expect(response.status).toBe(200);
...
test('post /restaurants - INCOMPLETE', async () => {
    try{
        await request(address).post('/restaurants').send({
...
test('patch /restaurants/:id', async () => {
    try{
        let postResponse = await request(address).post('/restaurants').send({
            name: 'RestaurantPatch'
        });

        let patchResponse = await request(address)
            .patch(`/restaurants/${postResponse.body._id}`).send({
...
test('patch /restaurants/xxx - NOT FOUND', async () => {
    try{
        return await request(address).patch('/restaurants/xxx');
    }catch(e){
        expect(e.status).toBe(404);
    }
});
...
test('delete /restaurants/xxx - NOT FOUND', async () => {
    try{
        let reponse = await request(address).delete('/restaurants/xxx');
    }catch(e){
        expect(e.status).toBe(404);
    }
})